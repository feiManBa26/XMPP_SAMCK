...
import com.yyquan.jzh.util.SweetAlertDialogListener;
import com.yyquan.jzh.util.SweetAlertDialogUtils;
import com.yyquan.jzh.util.ToastUtil;
import com.yyquan.jzh.view.DialogView;
import com.yyquan.jzh.xmpp.XMPPConnectionListener;
...
public class LoginActivity extends Activity implements View.OnClickListener, PlatformActionListener {
    ...
    private SweetAlertDialogUtils mSweetAlertDialogUtils;
...
//        lt = new Location(this);
        mSweetAlertDialogUtils = SweetAlertDialogUtils.builder()
               .setContext(this)
               .setMassage("哈哈哈")
               .setTouch(false)
               .setType(SweetAlertDialog.PROGRESS_TYPE)
               .setListener(new SweetAlertDialogListener() {
                   @Override
                   public void clickCancel() {
                       mSweetAlertDialogUtils.cancel();
                   }
                   @Override
                   public void ClickOk() {
                         mSweetAlertDialogUtils.cancel();
...
//        initialView();
    ...
    private static class MyHandler extends Handler {
        ...
        MyHandler(Activity activity) {
            this.mReference = new WeakReference<Context>(activity);
        }

        @Override
        public void handleMessage(Message msg) {
            LoginActivity activity = (LoginActivity) mReference.get();
            switch (msg.what) {
                case 3:
                    User user = (User) msg.obj;
                    activity.loginXmpp(user);
                    break;
            }
        }
    }
    ...
    private void loginXmpp(final User user) {
        ...
        try {
            XMPPTool.getInstance().connect(user.getUser(), user.getPassword(), new XMPPConnectionListener() {
                @Override
            ...
            XMPPTool.getInstance().authenticated();
...
//    };
    ...
    private void login(String user, String password) {
        Message m = mHandler.obtainMessage(XMPP_LOGIN);
        User users = new User();
        users.setUser(user);
                ...
                if (str != null) {
                    try {
                        ...
                        if (object.getString("code").equals("success")) {
                            ...
                            Message m = mHandler.obtainMessage(CREATE_USER);
                            m.obj = user;
                            mHandler.sendMessage(m);
